# app_ui.py
# FINAL version with all methods and UI logic restored.
...
STYLESHEET = """
    QMainWindow, #mainWidget { background-color: #FFFFFF; font-family: "Segoe UI"; }
...
class ProcessingWorker(QThread):
    ...
    def run(self):
        try:
            ...
            def combined_status_cb(category, message=""):
                self.status_updated.emit(message or category)
                if category == "SUCCESS": self.file_succeeded.emit()
                elif category == "FAIL": self.file_failed.emit()
                elif category == "NEEDS_REVIEW": self.needs_review_signal.emit()
                elif category == "OCR": self.ocr_used_signal.emit()

            updated_path, updated, failed = process_func(self.process_path, self.kb_path, self.progress_updated.emit, combined_status_cb, lambda: None, self.cancel_event)
...
class MainWindow(QMainWindow):
    ...
    def _setup_window_and_widgets(self):
        ...
        self.setMinimumSize(950, 700); self.resize(1100, 800)
        central_widget = QWidget(self); self.setCentralWidget(central_widget)
        self.main_layout = QVBoxLayout(central_widget)
        self.main_layout.setContentsMargins(0, 0, 0, 0); self.main_layout.setSpacing(0)
        self._create_header(); central_area = QWidget(); central_area_layout = QVBoxLayout(central_area)
        central_area_layout.setContentsMargins(15, 15, 15, 15); central_area_layout.setSpacing(15)
        self._create_io_section(central_area_layout); self._create_controls_section(central_area_layout)
        self._create_feedback_section(central_area_layout); self._create_log_section(central_area_layout)
        self.main_layout.addWidget(central_area, 1); self._create_status_bar()

    def _create_header(self):
        header_widget = QWidget(); header_widget.setObjectName("header"); layout = QHBoxLayout(header_widget)
        logo = QLabel("KYOCERA"); logo.setObjectName("logoLabel"); title = QLabel("QA ServiceNow Knowledge Tool"); title.setObjectName("titleLabel")
        layout.addWidget(logo); layout.addWidget(title); layout.addStretch(); self.main_layout.addWidget(header_widget)

    def _create_io_section(self, parent_layout):
        io_groupbox = QGroupBox("1. Select Files"); layout = QGridLayout(io_groupbox)
        self.kb_path_edit = QLineEdit(); self.kb_path_edit.setPlaceholderText("Click 'Browse...' to select your existing .xlsx file")
        kb_browse_btn = QPushButton("Browse..."); kb_browse_btn.clicked.connect(self.browse_kb_file)
        self.process_path_edit = QLineEdit(); self.process_path_edit.setPlaceholderText("Select a folder or a .zip file")
    ...
    def _create_controls_section(self, parent_layout):
        ...
        self.start_button.clicked.connect(self.start_processing); self.open_kb_button.clicked.connect(self.open_kb_file); self.clear_inputs_button.clicked.connect(self.clear_inputs)
    ...
    def _create_feedback_section(self, parent_layout):
        ...
        icon_layout = QHBoxLayout(); self.pass_indicator = StatusIndicator("Succeeded", "✅", "#00B176"); self.fail_indicator = StatusIndicator("Failed", "❌", "#E30613")
    ...
    def start_processing(self):
        ...
        self.worker.progress_updated.connect(self.update_progress); self.worker.status_updated.connect(self.update_status)
        self.worker.file_succeeded.connect(self.pass_indicator.increment); self.worker.file_failed.connect(self.fail_indicator.increment)
        self.worker.needs_review_signal.connect(self.review_indicator.increment); self.worker.ocr_used_signal.connect(self.ocr_indicator.increment)
        self.worker.processing_finished.connect(self.processing_finished); self.worker.processing_error.connect(self.handle_error)
        self.worker.finished.connect(lambda: self.update_ui_for_processing(False)); self.worker.start()
    ...
    def update_ui_for_processing(self, is_processing):
        ...
        if not is_processing: self.progress_bar.setValue(0)
        else:
    ...
    def processing_finished(self, updated_path, updated_count, failed_count):
        self.result_file_path = updated_path; self.log_message(f"Finished! {updated_count} records updated, {failed_count} files failed.")
        if QMessageBox.question(self, "Processing Complete", f"Success!\n- Records Updated: {updated_count}\n- Files Failed: {failed_count}\n\nWould you like to open the updated file?") == QMessageBox.Yes:
            self.open_kb_file()
